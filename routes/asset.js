...
exports.get = function(req, res) {
    ...
    }).populate('employee').exec(function(err, access_token) {
        var query = {};

        if (access_token && access_token.employee.user_role === 'employee') {
            query = {
                assignee: access_token.employee._id
            }
        }

        Asset.find(query).populate('assignee').exec(function(err, results) {
            if (err) {
...
exports.getId = function(req, res) {
    ...
    Asset.findOne({
        _id: req.params.id
    }).populate('assignee').exec(function(err, data) {
        if (err) {
            return res.end(JSON.stringify(err));
        }
...
exports.put = function(req, res) {
    ...
    Asset.findByIdAndUpdate(req.params.id, req.body, function(err, asset) {
        ...
        if (req.body.assignee) {
            ...
            Employee.findById(req.body.assignee).exec(function(err, employee) {
                if (err) {
                    return res.end(JSON.stringify(err));
                }
